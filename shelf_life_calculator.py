...
if uploaded_file is not None: try: data = pd.read_csv(uploaded_file) st.success("CSV loaded successfully.") except Exception as e: st.error(f"Error loading CSV: {e}")

elif manual_input: st.markdown("### 📝 Manual Input Table") with st.form("manual_form"): condition = st.selectbox("Stability Condition", ["25C_60RH", "30C_65RH", "40C_75RH"]) param_name = st.text_input("Parameter Name (e.g., Assay)", "Assay") timepoints = st.text_area("Enter Time Points (comma-separated)", "0,1,3,6,9,12") values = st.text_area("Enter Values (comma-separated)", "100,98,95,92,90,88") spec_limit = st.number_input("Specification Limit for Shelf Life Calculation", value=85.0, step=0.1) submit = st.form_submit_button("Add to Dataset")
...
def estimate_shelf_life_ich(x, stats=False, support_data=False, refrigerated=False, failure_month=None): formula = "" if failure_month is not None and failure_month <= x: formula = f"Product failed at {failure_month} months" return failure_month, formula if stats and support_data: if refrigerated: formula = "min(1.5 * x, x + 6)" return min(1.5 * x, x + 6), formula  # ICH case 2 else: formula = "min(2 * x, x + 12)" return min(2 * x, x + 12), formula  # ICH case 1 elif stats or support_data: if refrigerated: formula = "x + 3" return x + 3, formula  # ICH case 4 else: formula = "min(1.5 * x, x + 6)" return min(1.5 * x, x + 6), formula  # ICH case 3 else: formula = "x" return x, formula  # ICH case 5

if not data.empty: st.markdown("### 👁️ Data Preview") st.dataframe(data)
...
for (condition, param), df_group in data.groupby(["Condition", "Parameter"]):
    ...
    if slope != 0:
        ...
        if est_time > 0:
            ...
            ich_shelf, applied_formula = estimate_shelf_life_ich(
                x=est_time,
                stats=stats_flag,
            ...
            st.success(f"📉 ICH-Extrapolated Shelf-Life: **{ich_shelf:.2f} months** using `{applied_formula}`")
        else:
...
if results_summary: if st.button("📂 Download Combined Report as PDF"): pdf = FPDF() pdf.set_auto_page_break(auto=True, margin=15) pdf.add_page() pdf.set_font("Arial", 'B', 16) pdf.cell(0, 10, "Stability Study Summary Report", ln=True, align="C") pdf.ln(10) pdf.set_font("Arial", '', 12)
...
st.markdown("---") st.markdown("Built for Stability Analysis | Pharma Quality Tools")
