...
def estimate_shelf_life_ich(x, stats=False, support_data=False, refrigerated=False, failure_month=None): if failure_month is not None and failure_month <= x: return failure_month if stats and support_data: return min(2 * x, x + 12) if not refrigerated else min(1.5 * x, x + 6) elif stats or support_data: return min(1.5 * x, x + 6) if not refrigerated else x + 3 else: return x
